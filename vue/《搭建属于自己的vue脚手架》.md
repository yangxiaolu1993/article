...
在本地的某个路径下创建一个文件夹 xl-rookie-cli，在根目录下执行 npm init 创建 package.json，下面是配置好之后的内容。
...
```
执行 npm run dev，就可以把 ts 实时编译为 js ，并将编译好的 js 放到 outDir 配置的路径中。注意，在开发脚手架的过程中，npm run dev 一定要是执行的，毕竟 bin 执行文件对应的路径是编译之后的文件。

### 3) 编辑 bin 执行文件

唠叨了这么多，终于到了 bin 执行文件，bin 执行文件对应的是 package.json 中 bin 字段对应的文件，xl-rookie-cli 中的 ./dist/bin/index.js，也就是编译之前的 ./bin/index.ts。bin 执行文件中有一段很关键的代码，一定要放在文件开头
...
![](https://img13.360buyimg.com/imagetools/jfs/t1/203679/24/559/17160/611112ffE4c16b965/a3b10b268749b597.png)
...
![](https://img13.360buyimg.com/imagetools/jfs/t1/179210/13/18565/46456/611113bfEb88281d3/a950c8411a12f532.png)
...
如果你对上面这些依赖包的内容都已经熟悉了，小编相信，实现基础脚手架就是非常容易的事情，正式进入 bin 执行文件的编写。在 bin 执行文件中，添加 create 命令，来搭建一个项目。
...
```
其中，ROOT_CLI_PATH 函数用于定位到项目根目录。在 create 函数中，实现了搭建项目，其实思路很简单，就是在 GitHub 中按照自己的开发习惯创建一个项目模板，在 create 函数中，利用 download-git-repo 依赖包，将远程模板下载到当前目录就结束了，是不是 so easy.... 

按照这个思路，模板就可以有任意多个远程路径，在根目录下创建 template.json 文件，用于配置已有模板路径。
...
```
其中，normal 为模板名，[https://github.com:yangxiaolu1993/rookie-cli#rookie-cli-template](https://github.com:yangxiaolu1993/rookie-cli#rookie-cli-template) 为模板地址，模板地址要注意， [https://github.com:yangxiaolu1993/rookie-cli](https://github.com:yangxiaolu1993/rookie-cli) 为 GitHub 的地址，rookie-cli-template 为分支名称。
...
export async function create(program: {	args:string[] }) {
	// 读取根目录下的模板列表
...
* 注册好账号后，在终端执行命令 npm login，登录 npm 账号
...
证明安装成功。到了激动人心的时候了，使用 xl-rookie-cli 搭建项目，执行 

``` 
roockie-cli  create normal demo
```
...
执行 ```node ./index.js```，则会得到与执行 vue 一样的效果

![](https://img12.360buyimg.com/imagetools/jfs/t1/182650/32/18183/52819/610e26a2Efd9f0cad/8d0a5302f88eb13a.png)

还需要特别说明一下， program.args 用来获取执行命令传递的参数。比如：执行 vue init webpack demo 时，program.args 会以数组的形式存储参数

```
console.log(program.args[0]) // 输出 webpack
console.log(program.args[1]) // 输出 demo
```
...
如果远程仓库是 GitHub，repository 的格式是这样的：```https://github.com:远程仓库的名字#仓库分支```，其中仓库名称就是下图标识的位置。

![](https://img10.360buyimg.com/imagetools/jfs/t1/185697/4/18362/54906/61111aadE6f7b53a1/00a4ee3168b3ae87.png)
